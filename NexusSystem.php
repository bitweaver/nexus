...
class NexusSystem extends BitBase {
	...
	function loadPlugins( $pCacheTime=BIT_QUERY_CACHE_TIME ) {
		$rs = $this->getDb()->query( "SELECT * FROM `".BIT_DB_PREFIX."tiki_nexus_plugins`", NULL, BIT_QUERY_DEFAULT, BIT_QUERY_DEFAULT );
		while( $rs && !$rs->EOF ) {
	...
	function scanPlugins() {
		...
		foreach( array_keys( $this->mPlugins ) as $guid ) {
			...
			if( !isset( $handler['verified'] ) && $handler['is_active'] =='y' ) {
				...
				$this->getDb()->query( $sql, array( $guid, $handler['plugin_type'], $handler['description'] ) );
				$handler['is_active'] = 'y';
			}
		}
		if( !empty( $sql ) ) {
	...
	function storePluginSettings( $pParamHash ) {
		...
		$rs = $this->getDb()->query( "SELECT * FROM `".BIT_DB_PREFIX."tiki_nexus_plugin_settings`", NULL );
		while( $rs && !$rs->EOF ) {
			$settings[] = $rs->fields;
			$rs->MoveNext();
		}
