...
{if $gNexus->mInfo.tree}
	{form legend="Item Details"}
			...
			{if $item.last}</ul>{else}
				<li>
					<div class="{cycle values='even,odd'}">
						...
						<a href="{$smarty.const.NEXUS_PKG_URL}menu_items.php?menu_id={$item.menu_id}&amp;item_id={$item.item_id}">{biticon ipackage="icons" iname="accessories-text-editor" iexplain="edit item" style="float:right" iforce=icon}</a>
		...
		{if $gNexus->mInfo.tree}
			...
			<input type="image" src="{biticon ipackage="icons" iname="edit-delete" iexplain='remove item' url=TRUE}" title="remove selected items" style="float:right" />
		{/if}
	{/form}
{/if}
{/strip}
